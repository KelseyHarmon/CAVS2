...
document.addEventListener("DOMContentLoaded", () => {
  // Coordinates from the Google Maps iframe URL
  const latitude = 33.475847304016085;   // Extracted latitude
  const longitude = -88.7956568359317;   // Extracted longitude

  // Initialize the map and set its view to the extracted coordinates
  const map = L.map('map').setView([latitude, longitude], 13); // Set the initial map view
...
document.addEventListener("DOMContentLoaded", function () {
  // Parse vehicles from localStorage
  const vehicles = JSON.parse(localStorage.getItem('vehicles')) || [];

  // Check if vehicles exist
  if (vehicles.length === 0) {
    console.warn("No vehicles defined.");
    const outputDiv = document.getElementById('data-output');
    outputDiv.innerHTML = "No vehicles present. Waiting for connection...";
    return; // Stop further execution
  }

  // Extract the port number for the first vehicle
  const vehicle = vehicles[0]; // Use the first vehicle for simplicity
  const vehicleName = vehicle.name; // Example: "vehicle_name"
  const vehiclePort = vehicle.port; // Example: "9090"

  if (!vehiclePort) {
    console.error("Vehicle port is missing or invalid.");
    const outputDiv = document.getElementById('data-output');
    outputDiv.innerHTML = `Vehicle "${vehicleName}" has an invalid port.`;
    return;
  }

  console.log(`Connecting to vehicle: ${vehicleName} on port: ${vehiclePort}`);

  // Create the ROS connection using the port from the vehicle object
  const ros = new ROSLIB.Ros({
    url: `ws://localhost:${vehiclePort}` // Use the dynamic port
  });
  ...
  ros.on('connection', () => {
    ...
    const outputDiv = document.getElementById('data-output');
    outputDiv.innerHTML = `Connected to ROS for vehicle "${vehicleName}".`;
  });

  ros.on('error', (error) => {
    console.error('Error connecting to ROSBridge:', error);
    const outputDiv = document.getElementById('data-output');
    outputDiv.innerHTML = `Error: Could not connect to ROS for vehicle "${vehicleName}".`;
  });

  ros.on('close', () => {
    console.warn('Connection to ROSBridge was closed.');
    const outputDiv = document.getElementById('data-output');
    outputDiv.innerHTML = `Connection to ROS for vehicle "${vehicleName}" was closed.`;
  });
  ...
  setInterval(() => {
    if (ros.isConnected) {
      ...
      ros.callOnConnection({ op: 'ping' }); // Send a "ping" to keep the connection alive
    }
  }, 30000); // Send every 30 seconds

  // Create the topic for latitude and longitude
  const latTopic = new ROSLIB.Topic({
    ros: ros,
    name: '/latitude', // Topic name for latitude data
    messageType: 'std_msgs/Float64'
  });

  const lonTopic = new ROSLIB.Topic({
    ros: ros,
    name: '/longitude', // Topic name for longitude data
    messageType: 'std_msgs/Float64'
  });

  // Function to update the displayed data in the output console
  function updateDisplay(lat, lon) {
    const outputDiv = document.getElementById('data-output');
    outputDiv.innerHTML = `Latitude: ${lat}<br>Longitude: ${lon}`;
  }

  // Subscribe to the latitude and longitude topics
  latTopic.subscribe(function (message) {
    const latitude = message.data;
    console.log("Received Latitude:", latitude);
    const currentLongitude = document.getElementById('data-output').textContent || 0; // Get current longitude if available
    updateDisplay(latitude, currentLongitude); // Update with latitude and current longitude
  });

  lonTopic.subscribe(function (message) {
    const longitude = message.data;
    console.log("Received Longitude:", longitude);
    const currentLatitude = document.getElementById('data-output').textContent || 0; // Get current latitude if available
    updateDisplay(currentLatitude, longitude); // Update with current latitude and longitude
  });
});
